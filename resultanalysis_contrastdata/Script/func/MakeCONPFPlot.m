function MakeCONPFPlot(data, hf)
...
title(type);
saveName = [ 'All','_' data.type, '_', data.id, '_', data.subjectID, '_', data.date, '.fig'];
...
if strcmp(data.type , 'CON') 
    data.x.label = 'contrast magnitude';
    data.y.label = 'Proportion of correct';
    xlabel(data.x.label);
    ylabel(data.y.label);
    xlim([ min( data.x.value )-0.001, max( data.x.value ) + 0.001 ]); ylim([ 0, 1 + 0.01 ]);
%     % Legend
    ...
    for n = 1:length(data.all)
        if ~isnan(data.all.fitdata.isFit) %正答率のフィッティングの描画
            %フィッティング関数のデータ点
                ...
                xPlotIndexCorrect{n} = (min( data.x.value )-0.001:0.001:max( data.x.value )+0.001)';
...
%             end
            ...
            %閾値をプロット
                ...
                plot([ min( data.x.value )-0.001 max( data.x.value )+0.001 ], [0.75 0.75], 'Color','k','LineStyle' ,':');
